...
export default function TerminalBox() {
  ...
  const copyToClipboard = (text: any) => {
    navigator.clipboard.writeText('bugbreaker test.java');
    setIsClicked(true);
    setTimeout(() => setIsClicked(false), 400); // Remove border after 2 seconds
  };

  const terminalText = `user ~ % bugbreaker test.java`;
    ...
    <div className="max-w-2xl mx-auto rounded-lg shadow-lg overflow-hidden relative">
      ...
      <div className="bg-gray-900 text-white p-6 rounded-lg relative">
        ...
        <button
          ...
          className={` text-white font-bold p-2 rounded absolute right-2 bottom-3 ${isClicked ? 'border-2 border-white' : ''}`}
        >
